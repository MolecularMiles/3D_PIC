...
while i < n_atoms:
    ...
    if (x < float((n_x_points*x_spacing) - (2*wall_width*x_spacing))) & (x > float((wall_width*x_spacing))):
        if (y < float((n_y_points*y_spacing) - (2*wall_width*y_spacing))) & (y > float((wall_width*y_spacing))):
            if (z < float((n_z_points*z_spacing) - (2*wall_width*z_spacing))) & (z > float((wall_width*z_spacing))):
                i += 1
                temp = numpy.array([x])
...
print("Built the external magentic field...")
print(numpy.shape(b_z_lattice))
...
"""
print(charge_vector)
print(boundary_counter)
print("expect " + str((n_x_points-2)**3.0))
print(n_x_points**3.0)
"""
...
def grid_assign(electron_positions, ion_positions):
    for i in range(0, n_atoms):
        ...
        print(positions)
        """
        charge_vector[(n_x_points*y_1)+ x_1+ (n_x_points*n_x_points*z_1)] += ((w_1*e_charge)) 
        charge_vector[(n_x_points*y_2)+ x_2+ (n_x_points*n_x_points*z_2)] += ((w_2*e_charge))
        charge_vector[(n_x_points*y_3)+ x_3+ (n_x_points*n_x_points*z_3)] += ((w_3*e_charge))
        charge_vector[(n_x_points*y_4)+ x_4+ (n_x_points*n_x_points*z_4)] += ((w_4*e_charge))
        charge_vector[(n_x_points*y_5)+ x_5+ (n_x_points*n_x_points*z_5)] += ((w_5*e_charge)) 
        charge_vector[(n_x_points*y_6)+ x_6+ (n_x_points*n_x_points*z_6)] += ((w_6*e_charge))
        charge_vector[(n_x_points*y_7)+ x_7+ (n_x_points*n_x_points*z_7)] += ((w_7*e_charge))
        charge_vector[(n_x_points*y_8)+ x_8+ (n_x_points*n_x_points*z_8)] += ((w_8*e_charge))
...
def calculate_electric_field(charge_vector):
    ...
    for k in range(1, n_x_points-1):
        for j in range(1, n_x_points-1):
            for i in range(1, n_x_points-1):
                e_x_vector = numpy.append(e_x_vector, (pot_vector[(i-1) + (n_x_points*j) + (n_x_points*n_x_points*k)] - pot_vector[(i+1) + (n_x_points*j) + (n_x_points*n_x_points*k)])/(2*x_spacing*x_spacing))
                e_y_vector = numpy.append(e_y_vector, (pot_vector[(i) + (n_x_points*(j-1)) + (n_x_points*n_x_points*k)] - pot_vector[(i) + (n_x_points*(j+1)) + (n_x_points*n_x_points*k)])/(2*x_spacing*x_spacing))               
                e_z_vector = numpy.append(e_z_vector, (pot_vector[(i) + (n_x_points*j) + (n_x_points*n_x_points*(k-1))] - pot_vector[(i) + (n_x_points*j) + (n_x_points*n_x_points*(k+1))])/(2*x_spacing*x_spacing))
...
def get_accelerations(electron_positions, electron_velocities, ion_positions, ion_velocities, e_x, e_y, e_z, b_z):
    ...
    for i in range(0, n_atoms):
        ...
        e_x_felt = e_x[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_backwards_weight + e_x[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_backwards_weight + e_x[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_backwards_weight + e_x[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_forwards_weight + e_x[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_backwards_weight + e_x[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_forwards_weight + e_x[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_forwards_weight + e_x[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_forwards_weight
        e_y_felt = e_y[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_backwards_weight + e_y[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_backwards_weight + e_y[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_backwards_weight + e_y[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_forwards_weight + e_y[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_backwards_weight + e_y[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_forwards_weight + e_y[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_forwards_weight + e_y[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_forwards_weight   
        e_z_felt = e_z[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_backwards_weight + e_z[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_backwards_weight + e_z[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_backwards_weight + e_z[(x_int_index+1) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_forwards_weight*z_forwards_weight + e_z[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_backwards_weight + e_z[(x_int_index) + ((y_int_index+1)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_forwards_weight*z_forwards_weight + e_z[(x_int_index) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_backwards_weight*y*y_backwards_weight*z_forwards_weight + e_z[(x_int_index+1) + ((y_int_index)*n_x_points) + ((z_int_index+1)*n_x_points*n_x_points)]*x_forward_weight*y*y_backwards_weight*z_forwards_weight
...
with open("PIC_POSITIONS", "w") as f:
